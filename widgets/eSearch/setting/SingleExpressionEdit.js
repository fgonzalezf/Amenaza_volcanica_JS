...
function(declare, lang, array, html, query, on, json, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin,
  ...
  return declare([_WidgetBase,_TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    _bindEvents:function(){
      ...
      this.own(on(this.searchLabel, 'change', lang.hitch(this, this._checkProceed)));
    ...
    _checkProceed: function() {
      ...
      if(lang.trim(this.expressionAlias.get('value')) !== ''){
        canProceed = canProceed && this.searchLabel.get('value') !== '' && this.valuesTable.getData().length > 0;
      } else {
        canProceed = false;
      }
      if(lang.trim(this.expressionAlias.get('value')) === ''){
      ...
      if(lang.trim(this.searchLabel.get('value')) === ''){
        if(errormessage === ''){
          errormessage += this.nls.searchLabel + ' ' + this.nls.requiredfield;
        }else{
          errormessage += ', ' + this.nls.searchLabel + ' ' + this.nls.requiredfield;
        }
      }
